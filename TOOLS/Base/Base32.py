...
def base32_encrypt(plain_text):
    encoded_bytes = base64.b32encode(plain_text.encode("utf-8"))
    encoded_text = encoded_bytes.decode("utf-8")
    return encoded_text

def base32_decrypt(encoded_text):
    try:
        decoded_bytes = base64.b32decode(encoded_text.encode("utf-8"))
        decoded_text = decoded_bytes.decode("utf-8")
        return decoded_text
    except base64.binascii.Error:
        return "无效的 Base32 编码！"
...
def on_encrypt():
    ...
    if not text:
        ...
        return
    encrypted_text = base32_encrypt(text)
...
def on_decrypt():
    ...
    if not text:
        messagebox.showerror("输入为空！", "请输入待加/解密的内容！")
        return
    decrypted_text = base32_decrypt(text)
    result_text.delete(1.0, tk.END)
    result_text.insert(tk.END, "解密结果: " + decrypted_text)

root = tk.Tk()
root.title("Base32 加解密")
...
# 添加关于Base32加解密的介绍文字
base32_info_label = ttk.Label(frame, text="Base32是一种用32个字符来表示任意二进制数据的方法。\n在Base32中，将原始数据每5个字节一组，转换为8个字符。\nBase32编码后的数据长度通常会比原始数据增加约60%。")
base32_info_label.grid(row=3, column=0, columnspan=2, pady=(10,0))
